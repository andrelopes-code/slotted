...
export const REACT_BADGE_TOKENS = badgeTokens.map((name) => ({
  name,
  purpose: 'Theme-owned Badge decision',
}));

export const REACT_BADGE_DOCS = {
  badge: {
    api: apiRows([
      [
        'variant',
      ...
      ['fill', "'solid' | 'outline'", 'solid', 'Badge', 'How much of the tone is painted'],
      ['size', "'sm' | 'md'", 'md', 'Badge', 'Height and type size'],
      ['render', '(props) => ReactNode', '—', 'Badge', 'Renders a different root element'],
      ['className', 'string', '—', 'Badge', 'Additional class names'],
    ]),
    accessibility: [
    ...
    snippets: [
      defineSnippet({
        id: 'badge-react-status',
        language: 'tsx',
        label: 'A status in a table row',
        source: '<Badge variant="success">Paid</Badge>;',
      }),
      defineSnippet({
        id: 'badge-react-count',
        language: 'tsx',
        label: 'A count that needs words of its own',
        source: '<Badge aria-label="3 unread messages" size="sm" variant="danger">\n  3\n</Badge>;',
      }),
    ],
  },
} as const;
