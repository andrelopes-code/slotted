...
import { REACT_AVATAR_DOCS } from './avatar.docs';
...
describe('Avatar stories', () => {
  ...
  it('documents each public component API', () => {
    for (const member of Object.keys(contract.members) as Array<keyof typeof REACT_AVATAR_DOCS>) {
      expect(apiMetadataErrors(contract.members[member], REACT_AVATAR_DOCS[member].api)).toEqual(
        [],
      );
    }
  });
  ...
  it('keeps curated snippets formatted', async () => {
    const snippets = Object.values(REACT_AVATAR_DOCS).flatMap((docs) => docs.snippets);
