...
export const REACT_INPUT_TOKENS = inputTokens.map((name) => ({
  name,
...
export const REACT_INPUT_DOCS = {
  input: {
    api: apiRows([
      ['size', "'sm' | 'md' | 'lg'", 'md', 'Input', 'The control’s height and type scale'],
      [
        'disabled',
        'boolean',
        FROM_FIELD,
      ...
      ['invalid', 'boolean', FROM_FIELD, 'Input', 'Sets aria-invalid and the error appearance'],
      [
        'required',
        'boolean',
        FROM_FIELD,
      ...
      ['readOnly', 'boolean', FROM_FIELD, 'Input', 'Keeps the value visible and uneditable'],
      ['className', 'string', '—', 'Input', 'Additional class names'],
    ]),
    accessibility: [
