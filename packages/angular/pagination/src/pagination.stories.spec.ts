...
import { ANGULAR_PAGINATION_DOCS } from './pagination.docs';
import * as paginationStories from './pagination.stories';

describe('Angular Pagination stories', () => {
  ...
  it('documents each public component API', () => {
    for (const member of Object.keys(contract.members) as Array<
      keyof typeof ANGULAR_PAGINATION_DOCS
    >) {
      expect(
        apiMetadataErrors(contract.members[member], ANGULAR_PAGINATION_DOCS[member].api),
      ).toEqual([]);
    }
  });

  it('keeps curated Angular snippets formatted', async () => {
    const snippets = Object.values(ANGULAR_PAGINATION_DOCS).flatMap((docs) => docs.snippets);
