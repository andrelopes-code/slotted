...
export const ANGULAR_TEXTAREA_TOKENS = textareaTokens.map((name) => ({
  name,
...
export const ANGULAR_TEXTAREA_DOCS = {
  textarea: {
    api: apiRows([
      ['size', "'sm' | 'md' | 'lg'", 'md', 'slTextarea', 'The control’s padding and type scale'],
      ['rows', 'number', '3', 'slTextarea', 'The smallest the control gets, in lines'],
      [
        'autoSize',
        'boolean',
        'false',
      ...
      [
        ...
        FROM_FIELD,
        'slTextarea',
        'Sets aria-invalid and the error appearance',
      ],
      [
        'required',
        'boolean',
      ...
      ['readOnly', 'boolean', FROM_FIELD, 'slTextarea', 'Keeps the value visible and uneditable'],
    ]),
    accessibility: [
